    ...
    public class ContaController : Controller
    {
        ...
        public async Task<ActionResult> Registrar(ContaRegistrarViewModel modelo)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("", "As senhas informadas não são iguais");
                    return View(modelo);
                }
        ...
        public async Task<ActionResult> Login(ContaLoginViewModel modelo)
        {
            if (ModelState.IsValid)
            {
                ...
                var signInResultado = await SignInManager.PasswordSignInAsync(usuario.UserName,
                                                    modelo.Senha,
            ...
            if (ModelState.IsValid)
            {
                ...
                if (usuario != null)
                {
                    ...
                    await UserManager.SendEmailAsync(
                        ...
                        $"Clique aqui {linkDeCallback} para alterar a senha");
        ...
        public ActionResult VerificaCodigoCelular()
        {
            return View();
        }

        [HttpPost]
        public async Task<ActionResult> VerificaCodigoCelular(string token)
        {
            var usuarioId = HttpContext.User.Identity.GetUserId();
            var usuario = await UserManager.FindByIdAsync(usuarioId);
        ...
        [HttpPost]
        public async Task<ActionResult> DeslogarDeTodosLocais()
        {
            var usuarioId = HttpContext.User.Identity.GetUserId();
            var resultado = await UserManager.UpdateSecurityStampAsync(usuarioId);
        ...
        private async Task EnviarSmsDeConfirmacaoAsync(UsuarioAplicacao usuario)
        {
            ...
            await UserManager.SendSmsAsync(
                usuario.Id,
                $"Token de confirmação {tokenDeConfirmacaco}");
        }
    }
}